...
void changeLEDState(const char * LEDStateData, size_t length) { //What happens when the ESP32 receives a instruction from the server (with variable) to change the LED
  ...
  int LEDStatus = dataString.toInt(); //When we have a string we can use the built in Arduino function to convert to an integer

  Serial.print("Dette er LEDStatus i INT: "); //We print the final state
  Serial.println(LEDStatus);
  digitalWrite(gul_LED, LEDStatus);//We now use the varible to change the light (1 is on, 0 is off)
}
