var socket = io.connect('ws://192.168.2.139:2520', {secure: false});  //This line declares a socket.io object to var "socket" and connects to the server (c
// hange the IP-address and port to your own)
...
socket.on('connect',function() { //When you connect to the server (and it works) call this function
    console.log('Klient har koblet til serveren'); //The client prints this message
}); //The 'connect' function/identifier  is the standard procedure. To make something more we have to make it ourselves


socket.on('clientConnected',function(id, ip) { //This is our selfmade functions. Heno
    console.log('Klient ID: ' + id); //In this case the server will tell us what our local ID is (auto assigned)
    console.log("Klient IP: " + ip);//And it will tell us what our IP-address
...
socket.on('data', function(data) { //Received data from the server who is forwarding it to us from the ESP32
    ...
    meldingArr1.push([hentTidsstempel(), ("[SERVER] " + data)]);
    fyllKonsoll();
...
socket.on('melding', function(melding) { //Received data from the server who is forwarding it to us from the ESP32
    console.log('Melding er mottatt: ' + melding);
});

function hentTidsstempel() {
    var d = new Date();
    var dates = d.getHours() + ":" + ('0'+d.getMinutes()).slice(-2);
    return dates;
}
...
function changeLEDState(state) {
    ...
    console.log("endreLEDStatus er kalt");
...
function endreStatusDoor() {
    ...
    socket.emit('endreStatusDoor', holdStatus);
    var status = document.getElementById("holdDoor");
    
    if(status.style.backgroundColor == "red") {
        status.style.backgroundColor = "green";
        status.innerHTML = "Ikke hold dør";
        meldingArr1.push([hentTidsstempel(), "[KLIENT] Sendte forespørsel om å holde dør til server"]);
    } else {
        status.style.backgroundColor = "red";
        status.innerHTML = "Hold dør";
        meldingArr1.push([hentTidsstempel(), "[KLIENT] Sendte forespørsel om å ikke holde opp dør til server"]);
    }
    fyllKonsoll();
};
...
function endreLaasDoor() {
    ...
    socket.emit('endreLaasDoor', laasStatus);
    var laas = document.getElementById("laasDoor");
    if(laas.style.backgroundColor == "red") {
        laas.style.backgroundColor = "green";
        laas.innerHTML = "Lås opp dør";
        meldingArr1.push([hentTidsstempel(), "[KLIENT] Sendte forespørsel om å låse dør til server"]);
    } else {
        laas.style.backgroundColor = "red";
        laas.innerHTML = "Lås dør";
        meldingArr1.push([hentTidsstempel(), "[KLIENT] Sendte forespørsel om å låse opp dør til server"]);
    }
    fyllKonsoll();
};

function fyllKonsoll(){
    document.getElementById("botContainer").innerHTML = "";
    for(i = 0; i < meldingArr1.length; i++) {
        document.getElementById("botContainer").innerHTML += "<p class=\"melding\">" + meldingArr1[i][0] + " -> " + meldingArr1[i][1] + "</p><br>";
    };
};
...
//This means we dont have to use the delay() function or the millis() function in Arduino, we can just let No de and JavaScript fix the tracking of time for us
...
function requestDataFromBoard(interval) {
    ...
    console.log("foresporDataFraESP ble kalt med intervall: " + interval);
